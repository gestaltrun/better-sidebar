...
- **🌐 Embedded Browser**: multiple web tabs with back / forward / refresh; content runs in a sandboxed iframe. HTTP and HTTPS links open in the sidebar by default; each protocol can be configured in settings.
...
- **📂 Model-driven sidebar opens**: the `sidebar_open` tool is enabled by default and lets the model open files / folders (tree rooted there) / HTTP(S) pages in the sidebar. It can be disabled in global settings.
