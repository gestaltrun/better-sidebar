...
describe('findProfileDir / findPluginRoot', () => {
  ...
  beforeEach(() => {
    ...
    const pkgRoot = join(root, 'profiles', 'web', 'node_modules', '.pnpm', '@gestaltrun+dsh-better-sidebar@0.0.0', 'node_modules', '@gestaltrun', 'dsh-better-sidebar')
    ...
    writeFileSync(join(pkgRoot, 'package.json'), JSON.stringify({ name: '@gestaltrun/dsh-better-sidebar' }))
  ...
  it('walks up to the plugin package root by package.json name', () => {
    expect(findPluginRoot(moduleFile)).toBe(realpathSync(join(root, 'profiles', 'web', 'node_modules', '.pnpm', '@gestaltrun+dsh-better-sidebar@0.0.0', 'node_modules', '@gestaltrun', 'dsh-better-sidebar')))
  })
...
describe('depsStatus', () => {
  ...
  it('reports the cause, the repair command and the profile when the load fails', () => {
    ...
    const script = realpathSync(join(root, 'profiles', 'web', 'node_modules', '.pnpm', '@gestaltrun+dsh-better-sidebar@0.0.0', 'node_modules', '@gestaltrun', 'dsh-better-sidebar', 'scripts', scriptName))
